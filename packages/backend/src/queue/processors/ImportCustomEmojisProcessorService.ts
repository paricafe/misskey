...
// meta.json 中の絵文字名: Unicode の文字・数字・結合文字と _+- からなる名前
const EMOJI_NAME_PATTERN = /^[\p{Letter}\p{Number}\p{Mark}_+-]+$/u;
		...
		try {
			...
			try {
				...
				for (const record of meta.emojis as EmojiRecord[]) {
					...
					const emojiName = typeof record.emoji?.name === 'string' ? record.emoji.name.normalize('NFC') : record.emoji?.name;
					if (!isValidName(emojiName, EMOJI_NAME_PATTERN)) {
						this.logger.error(`invalid emojiname: ${truncateForLog(record.emoji?.name)}`);
						continue;
					}
					record.emoji.name = emojiName;
